...
커뮤니티 검색 탭은 사용자가 입력한 검색어를 바탕으로 네이버 웹 검색 결과를 조회하고, 자동차 유지비와 연비에 관련된 커뮤니티 글 링크만 골라 `community_posts` 테이블에 저장합니다.

동작 흐름:

1. 사용자가 커뮤니티 검색 탭에서 검색어를 입력합니다. 기본 검색어는 `연비 좋은 차`입니다.
2. `검색/크롤링` 버튼을 누르면 기존 `community_posts` 테이블 데이터를 먼저 삭제합니다.
3. `app/sources.py`의 `search_community_posts()` 함수가 네이버 웹 검색 페이지(`https://search.naver.com/search.naver`)에 요청을 보냅니다.
4. 검색 대상은 아래 3개 자동차/생활 커뮤니티로 제한합니다.
   - 보배드림: `bobaedream.co.kr`
   - 클리앙: `clien.net`
   - 뽐뿌: `ppomppu.co.kr`
5. 입력 검색어에 아래 보조 키워드를 붙여 사이트별로 검색합니다.
   - `연비`
   - `유지비`
   - `기름값 절약`
   - `주유비 절약`

예를 들어 검색어가 `아반떼`이면 내부적으로 아래와 같은 검색어를 만듭니다.

```text
아반떼 연비 site:bobaedream.co.kr
아반떼 유지비 site:bobaedream.co.kr
아반떼 기름값 절약 site:bobaedream.co.kr
아반떼 주유비 절약 site:bobaedream.co.kr
```

각 사이트마다 같은 방식으로 검색하므로 현재 기준으로는 `3개 사이트 x 4개 보조 키워드 = 12회` 검색 요청을 수행합니다.

검색 결과 필터링:

- 검색 결과 HTML에서 모든 링크(`a` 태그)를 읽습니다.
- 제목과 URL이 없는 링크는 제외합니다.
- 제목에서 날짜, 괄호, 대괄호 문구를 제거해 정리합니다.
- `이미지`, `뉴스`, `쇼핑`, `블로그`, `카페`, `지식iN`, `최신순`, `관련도순` 등 검색 UI나 일반 검색 카테고리에 가까운 결과는 제외합니다.
- 제목에 사용자 검색어, `연비`, `유지비`, `기름값` 중 하나도 없으면 제외합니다.
- URL에 대상 커뮤니티 도메인이 포함되지 않으면 제외합니다.

관련도 정렬:

- 제목에 포함된 단어에 따라 관련도 점수를 계산합니다.
- `연비`, `유지비`, `기름값`, `주유비`는 높은 점수를 줍니다.
- `전비`, `충전비`, `후기`, `주행`, `운행`, `절약`, `하이브리드`, `전기차`도 관련 키워드로 가중치를 줍니다.
- URL 중복과 제목 중복을 제거한 뒤 관련도 점수가 높은 순서로 최대 20개만 저장합니다.
...
source       검색 출처 사이트명
title        검색 결과 제목
url          게시글 또는 검색 결과 URL
snippet      요약문, 현재 구현에서는 빈 문자열
keyword      사용자가 입력한 검색어
published_at 게시일, 현재 구현에서는 NULL
crawled_at   수집 시각
```

현재 구현은 커뮤니티 본문을 직접 대량 수집하지 않고, 공개 검색 결과의 제목과 URL 중심으로 저장합니다. 네이버 검색 화면 구조가 바뀌면 선택자나 필터링 로직을 조정해야 할 수 있습니다.
